...
if [ "$GITHUB_EVENT_NAME" == "push" ] || [ "$GITHUB_EVENT_NAME" == "pull_request" ]; then
	...
	if test -n "$(find $ARTIFACTS_PATH -maxdepth 1 -name "*_${BASE_SHA}_*" -print -quit)"; then
		...
		git stash --include-untracked && git -c core.hooksPath=/dev/null checkout --quiet $GITHUB_SHA > /dev/null && echo 'On' $(git rev-parse HEAD)
	...
	DELTAS=$(echo $PERF_RESULTS | grep 'delta:' | sed -e 's/[^-0-9. ]//g')
	DELTAS_ARRAY=($result)
