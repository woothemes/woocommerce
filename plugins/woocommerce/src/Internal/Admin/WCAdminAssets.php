...
class WCAdminAssets {
	 ...
	 * @param bool   $requires_admin_app Whether the script requires the admin app. Defaults to true.
	 */
	public static function register_script( $script_path_name, $script_name, $need_translation = false, $dependencies = array(), $requires_admin_app = true ) {
		$script_assets_filename = self::get_script_asset_filename( $script_path_name, $script_name );
		$script_assets          = require WC_ADMIN_ABSPATH . WC_ADMIN_DIST_JS_FOLDER . $script_path_name . '/' . $script_assets_filename;
		$base_dependencies      = $requires_admin_app ? array( WC_ADMIN_APP ) : array();

		wp_enqueue_script(
			'wc-admin-' . $script_name,
			self::get_url( $script_path_name . '/' . $script_name, 'js' ),
			array_merge( $base_dependencies, $script_assets ['dependencies'], $dependencies ),
