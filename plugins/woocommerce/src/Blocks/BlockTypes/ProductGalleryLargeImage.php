	...
	protected function render( $attributes, $content, $block ) {
		...
		return strtr(
			'<div class="wc-block-product-gallery-large-image wp-block-woocommerce-product-gallery-large-image" {directives}>
				<ul class="wc-block-product-gallery-large-image__container swiper-wrapper" tabindex="-1">
					{main_images}
				</ul>
					{content}
			</div>',
	...
	private function get_main_images_html( $context, $product_id ) {
		...
		$main_image_with_wrapper = array_map(
			function ( $main_image_element ) {
				return "<li class='wc-block-product-gallery-large-image__wrapper swiper-slide'>" . $main_image_element . '</li>';
			},
			$main_images
		);
