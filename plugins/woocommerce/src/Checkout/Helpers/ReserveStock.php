...
final class ReserveStock {
	...
	private function get_query_for_reserved_stock( $product_id, $exclude_order_id = 0 ) {
		...
		$where_status = "posts.post_status IN ( '" . OrderLegacyStatus::PENDING . "', '" . OrderLegacyStatus::DRAFT . "' )";
		if ( OrderUtil::custom_orders_table_usage_is_enabled() ) {
			$join         = "{$wpdb->prefix}wc_orders orders ON stock_table.`order_id` = orders.id";
			$where_status = "orders.status IN ( '" . OrderLegacyStatus::DRAFT . "', '" . OrderLegacyStatus::PENDING . "' )";
		}
